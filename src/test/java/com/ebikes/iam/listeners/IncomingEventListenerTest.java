...
class IncomingEventListenerTest {
  ...
  private static final String ROUTING_KEY = "iam.user-extension.created";
  ...
  void shouldDiscardWhenRoutingKeyMissing() {
  ...
  void shouldDiscardWhenRoutingKeyBlank() {
  ...
  void shouldDiscardWhenNoHandlerMatches() {
  ...
  void shouldInvokeMatchingHandler() {
  ...
  void shouldSwallowHandlerException() {
    when(handler.matches(ROUTING_KEY)).thenReturn(true);
